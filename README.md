...
### Lending Curator Dashboard

Operational dashboard for Stake DAO's lending product, sole UI consumer of a closed-source off-chain stack I architected end-to-end (Envio indexer, Bun API with an alert engine, per-block liquidation bot). Real-time staleness threading per widget, virtualized high-cardinality tables, 10+ views covering markets, positions, liquidations, oracles, pools, wrappers, and vaults.

[View Live](https://lending-dashboard.stakedao.org/)
...
- [**Universal Composable Router**](https://github.com/stake-dao/contracts-monorepo/tree/main/packages/periphery/src/router) · Stateless modular entrypoint for the Stake DAO ecosystem with 10+ plug-and-play modules and permit2-only flow, deployed on 7+ networks
- **Lending Off-Chain Stack** · Closed-source backend powering the curator dashboard: Envio event indexer, Bun API with atomic snapshot polling and an alert engine, per-block liquidation bot with Flashbots and pricer fallback chain
